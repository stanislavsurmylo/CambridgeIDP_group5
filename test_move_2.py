...
def rotate_left_angle(mL, mR, speed, ms):
    # Left wheel backward, right wheel forward
    mL.Reverse(speed)
    mR.Forward(speed)
    sleep(ms / 1000)
...
def rotate_right_angle(mL, mR, speed, ms):
    # Left wheel forward, right wheel backward
    mL.Forward(speed)
    mR.Reverse(speed)
    sleep(ms / 1000)
...
def full_rotation(on1, on2, on3, on4, rule):
    if rule == 'R' and on4:
        shift_forward(motorL, motorR, SPEED_FWD, SHIFT_MS)
        rotate_right_angle(motorL, motorR, SPEED_TURN, )
        stop_all(motorL, motorR)
        return 'R'
    elif rule == 'L' and on1:
        shift_forward(motorL, motorR, SPEED_FWD, SHIFT_MS)
        rotate_left_angle(motorL, motorR, SPEED_TURN, 1)
        stop_all(motorL, motorR)
        return 'L'
    elif rule == 'S':
...
def test_move():
    ...
    motorR = Motor(dirPin=4, PWMPin=5)  # LEFT  motor on GP4/GP5
    motorL = Motor(dirPin=7, PWMPin=6)  # RIGHT motor on GP6/GP7

    while True:
        # Read raw sensor values (0=line, 1=not line)
        # Convert to booleans: True when on line
        on1  = is_line(sensor_bright.value())
        on2   = is_line(sensor_fleft.value())
        on3  = is_line(sensor_fright.value())
        on4   = is_line(sensor_bleft.value())
        
        print(f"BR:{on_bright} BL:{on_bleft} FR:{on_fright} FL:{fleft}")

        line_follow(on1, on2, on3, on4)
        if full_rotation(on1, on2, on3, on4, BRANCH_ROUTE[branch_idx]) in ['R','L','S']:
            branch_idx += 1
